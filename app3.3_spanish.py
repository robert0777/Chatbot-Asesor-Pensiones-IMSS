...
# Initialize NVIDIA AI client - EXACTLY as in their documentation
try:
    nvidia_client = OpenAI(
        base_url="https://integrate.api.nvidia.com/v1",
        api_key=os.getenv("NVIDIA_API_KEY")  # Make sure this matches your .env
    )
    ...
    st.error(f"Failed to initialize NVIDIA client: {str(e)}")
    st.stop()
...
# MODIFIED Query Processing section:
if prompt1:
        ...
        if "documents" in st.session_state:
            try:
                with st.spinner('Analizando documentos...'):
                    ...
                    # Updated to match NVIDIA's exact API format
                    completion = nvidia_client.chat.completions.create(
                        ...
                        top_p=1,
                        max_tokens=1024,
                        stream=False  # Changed to False for Streamlit compatibility
                    )
